...
void solve(int ct , int length, char inputBuffer[], char *args[],int *background ){
	...
	if( length > 3 && inputBuffer[0] == 'r' && inputBuffer[1] == ' ' ){
		bool isHistoryCommand = true;
		for(int loop = 3 ; loop < length ; loop++){
			if(inputBuffer[loop] == ' ' || inputBuffer[loop] == '\t' 
				|| inputBuffer[loop] == '\n'){
				continue;
			}
			else{
		...
		if(isHistoryCommand){
			char firstLetter = inputBuffer[2];
			char *tempBuffer = getCommand(firstLetter) ; 
			if(tempBuffer == NULL){
				printf("\nno such command\n");
			}
			else{
